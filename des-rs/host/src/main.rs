...
const DES_TEST_BUFFER_SIZE: usize = 4096;
const DES_TEST_KEY_SIZE: usize = 8; // DES密钥大小为8字节
const DES_BLOCK_SIZE: usize = 8;   // DES块大小为8字节

const DECODE: i8 = 0;
const ENCODE: i8 = 1;

fn prepare_des(session: &mut Session, encode: i8) -> optee_teec::Result<()> {
    let p2_value = if encode == ENCODE {
        Mode::Encode as u32
    } else {
        Mode::Decode as u32
    };
    let p0 = ParamValue::new(Algo::DES as u32, 0, ParamType::ValueInput); // 使用DES算法
    let p1 = ParamValue::new(KeySize::Bit64 as u32, 0, ParamType::ValueInput); // DES密钥大小为64位
...
fn main() -> optee_teec::Result<()> {
    ...
    let key = [0xa5u8; DES_TEST_KEY_SIZE];
    let iv = [0x00u8; DES_BLOCK_SIZE];
    let clear = [0x5au8; DES_TEST_BUFFER_SIZE];
    let mut ciph = [0x00u8; DES_TEST_BUFFER_SIZE];
    let mut tmp = [0x00u8; DES_TEST_BUFFER_SIZE];

    println!("Prepare encode operation");
    prepare_des(&mut session, ENCODE)?;
    ...
    prepare_des(&mut session, DECODE)?;
